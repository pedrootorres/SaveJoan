...
function Update () {
	...
	if (Physics.Raycast(ray,hit,bulletSpeed*Time.deltaTime) && !hit.collider.name.Contains("Attack Point") && !hit.collider.name.Contains("No Turrets Here")){
		
		//  THIS PART OF THE CODE IS MINE!!!  //
		if(hit.collider.gameObject.name == "Spider(Clone)") {
			hit.collider.GetComponent("Spider").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		} else if(hit.collider.gameObject.name == "Troll(Clone)") {
			hit.collider.GetComponent("Troll").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		} else if (hit.collider.gameObject.name == "Robot(Clone)") {
			hit.collider.GetComponent("Robot").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		} else if (hit.collider.gameObject.name == "Demon(Clone)") {
			hit.collider.GetComponent("Demon").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		} else if (hit.collider.gameObject.name == "Alien(Clone)") {
			hit.collider.GetComponent("Alien").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		} else if(hit.collider.gameObject.name == "Barrel") {
			hit.collider.transform.parent.GetComponent("Barrel").GetHit(GameObject.Find("Player(Clone)").GetComponent("Player").damage);
		}
		// THIS PART OF THE CODE IS MINE!!!  //
