...
public class Turret4 : MonoBehaviour {
	...
	void Start () {
		...
		flashGenerator = this.transform.FindChild ("Head001").FindChild ("flashGenerator");
		traceGenerator = this.transform.FindChild ("Head001").FindChild ("traceGenerator");
	}
	...
	void turretAI() {
		...
		// Look at the target
		if(foundTarget){
			this.transform.FindChild("Head001").LookAt(targetTransform);
			if(!alreadyShooting) {
				StartCoroutine(Shoot(hits[shootMe]));
			}
		} else {
			this.transform.FindChild("Head001").LookAt(null);
